...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['chief-nutritionist', 'consultant'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<FunctionalNutritionistInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/functional-nutritionists` : null),
    () => getFunctionalNutritionists({ tenant_id: session?.user?.tenantId }),
  );
  loading = isLoading;
  ...
  const LinkItems: Array<LinkItemProps> = [
    ...
    {
      name: 'Functional Nutritionist',
      path: isTenantUser ? `/functional-nutritionists/view/${data?.[0]?.id}` : '/functional-nutritionists',
