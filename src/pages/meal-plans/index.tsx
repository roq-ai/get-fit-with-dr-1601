...
function MealPlanListPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<MealPlanInterface[]>(
    ...
    () =>
      getMealPlans({
        relations: ['functional_nutritionist', 'patient'],
      }),
  );
            ...
            <Table variant="simple">
              <Thead>
                <Tr>
                  <Th>name</Th>
                  <Th>description</Th>
                  {hasAccess('functional_nutritionist', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                    <Th>functional_nutritionist</Th>
                  )}
                  ...
                  <Tr cursor="pointer" onClick={() => handleView(record.id)} key={record.id}>
                    ...
                    {hasAccess('functional_nutritionist', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                      <Td>
                        <Link
                          as={NextLink}
                          href={`/functional-nutritionists/view/${record.functional_nutritionist?.id}`}
                        >
                          {record.functional_nutritionist?.name}
                        </Link>
                      </Td>
                    )}
